...
//?: вли€ет ли регистр слова на поиск - не вли€ет
    ...
    void find() 
    {
        if (word == "") {
            ...
            for (unsigned int i = 0; i < count; i++) {
                ...
                int c = phrases[i].find(word); //работает дл€ совпадени€: фраза: покакает, слово: пока, совпадение будет
                if (c == -1) {
                    if (i == (count-1)) {
                        a_count = count - new_count - 1;
                        //cout << "Ёто а_каунт: " << a_count;
                    }
                    continue;
                }
                else if (c != -1) {
